...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (timedOut) {
      console.warn(`[API Timeout] Notification logs query timed out after 2000ms. Response time: ${responseTimeMs}ms`);
      return NextResponse.json({
        logs: [],
        warning: 'Database query timed out; no logs available',
        timeout: true
